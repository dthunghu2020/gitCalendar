...
public class SingleMonthView extends MonthView {
     ...
     *Nếu bạn cần nhấp vào Scheme để không có hiệu lực, hãy trả về true
     *
     * @param canvas    canvas
     * @param calendar  日历日历calendar
     * @param x         日历Card x起点坐标      Thẻ lịch x tọa độ điểm bắt đầu
     * @param y         日历Card y起点坐标      Thẻ Lịch y tọa độ điểm bắt đầu
     * @param hasScheme hasScheme 非标记的日期  không đánh dấu ngày
     * @return false 则不绘制onDrawScheme，因为这里背景色是互斥的
     * false không vẽ trênDrawScheme, vì màu nền ở đây là loại trừ lẫn nhau
     */
    @Override
...
//        int cy = y + mItemHeight / 2;
    ...
    protected void onDrawText(Canvas canvas, Calendar calendar, int x, int y, boolean hasScheme, boolean isSelected) {
        ...
        int cx = x + mItemWidth / 2;
        if (isSelected) {
            canvas.drawText(calendar.isCurrentDay() ? "chưa chọn" : "đã chọn",
                    cx,
                    baselineY,
                    mSelectTextPaint);
        } else if (hasScheme) {
            canvas.drawText(calendar.isCurrentDay() ?
                            "chưa chọn" : String.valueOf(calendar.getDay()),
                    cx,
                    baselineY,
