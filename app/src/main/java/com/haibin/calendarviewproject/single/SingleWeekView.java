...
//        int cy = mItemHeight / 2;
    ...
    protected void onDrawText(Canvas canvas, Calendar calendar, int x, boolean hasScheme, boolean isSelected) {
        ...
        int cx = x + mItemWidth / 2;
        if (isSelected) {
            canvas.drawText(calendar.isCurrentDay() ? "chưa chọn" : "đã chọn",
                    cx,
                    baselineY,
                    mSelectTextPaint);
        } else if (hasScheme) {
            canvas.drawText(calendar.isCurrentDay() ? "chưa chọn" : String.valueOf(calendar.getDay()),
                    cx,
                    baselineY,
