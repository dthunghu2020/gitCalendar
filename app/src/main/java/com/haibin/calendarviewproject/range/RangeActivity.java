...
public class RangeActivity extends BaseActivity implements
    ...
    protected void initData() {
        ...
        map.put(getSchemeCalendar(year, month, 3, 0xFF40db25, "three").toString(),
                getSchemeCalendar(year, month, 3, 0xFF40db25, "three"));
        map.put(getSchemeCalendar(year, month, 6, 0xFFe69138, "six").toString(),
                getSchemeCalendar(year, month, 6, 0xFFe69138, "six"));
        map.put(getSchemeCalendar(year, month, 9, 0xFFdf1356, "nine").toString(),
                getSchemeCalendar(year, month, 9, 0xFFdf1356, "nine"));
        map.put(getSchemeCalendar(year, month, 13, 0xFFedc56d, "thirteen").toString(),
                getSchemeCalendar(year, month, 13, 0xFFedc56d, "thirteen"));
        map.put(getSchemeCalendar(year, month, 14, 0xFFedc56d, "fourteen").toString(),
                getSchemeCalendar(year, month, 14, 0xFFedc56d, "fourteen"));
        map.put(getSchemeCalendar(year, month, 15, 0xFFaacc44, "fifteen").toString(),
                getSchemeCalendar(year, month, 15, 0xFFaacc44, "fifteen"));
        map.put(getSchemeCalendar(year, month, 18, 0xFFbc13f0, "eighteen").toString(),
                getSchemeCalendar(year, month, 18, 0xFFbc13f0, "eighteen"));
        map.put(getSchemeCalendar(year, month, 25, 0xFF13acf0, "twenty-five").toString(),
                getSchemeCalendar(year, month, 25, 0xFF13acf0, "twenty-five"));
        map.put(getSchemeCalendar(year, month, 27, 0xFF13acf0, "twenty-seven").toString(),
                getSchemeCalendar(year, month, 27, 0xFF13acf0, "twenty-seven"));
    ...
    private Calendar getSchemeCalendar(int year, int month, int day, int color, String text) {
        ...
        calendar.addScheme(0xFF008800, "three");
        calendar.addScheme(0xFF008800, "fourteen");
        return calendar;
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.iv_clear:
                ...
                mTextLeftWeek.setText("ngày bắt đầu");
                mTextRightWeek.setText("Ngày cuối");
            ...
            case R.id.tv_commit:
                ...
                Toast.makeText(this, String.format("đã chọn %s ngày: %s —— %s", calendars.size(),
    ...
    public void onCalendarInterceptClick(Calendar calendar, boolean isClick) {
        Toast.makeText(this,
                calendar.toString() + (isClick ? "Chặn không thể nhấp" : "Chặn được đặt thành ngày không hợp lệ"),
                Toast.LENGTH_SHORT).show();
    }
    ...
    public void onCalendarSelectOutOfRange(Calendar calendar) {
        // TODO: 2018/9/13 超出范围提示 Ra khỏi phạm vi nhắc
    }

    @Override
    public void onSelectOutOfRange(Calendar calendar, boolean isOutOfMinRange) {
        Toast.makeText(this,
                calendar.toString() + (isOutOfMinRange ? "Ít hơn phạm vi lựa chọn tối thiểu" : "Vượt quá phạm vi lựa chọn tối đa"),
                Toast.LENGTH_SHORT).show();
    }
    ...
    public void onCalendarRangeSelect(Calendar calendar, boolean isEnd) {
        if (!isEnd) {
            mTextLeftDate.setText(calendar.getMonth() + "tháng" + calendar.getDay() + "ngày");
            mTextLeftWeek.setText(WEEK[calendar.getWeek()]);
            mTextRightWeek.setText("Ngày cuối");
            mTextRightDate.setText("");
        } else {
            mTextRightDate.setText(calendar.getMonth() + "tháng" + calendar.getDay() + "ngày");
            mTextRightWeek.setText(WEEK[calendar.getWeek()]);
        }
    }

    private static final String[] WEEK = {"sun", "mun", "tue", "wed", "thu", "fri", "sat"};
}
